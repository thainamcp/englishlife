...
struct CharactersListView: View {
  ...
  private var emptyState: some View {
    VStack(spacing: 18) {
      ...
      ).foregroundStyle(ThemeApp.Colors.textSecondary).multilineTextAlignment(.center).padding(
        .horizontal, 38)
            ...
            GlassCard {
              HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                  Text(character.name).font(ThemeApp.Fonts.ctaButton()).foregroundStyle(
                ...
                Spacer()
                Image(systemName: "chevron.right").foregroundStyle(ThemeApp.Colors.textSecondary)
              }
            }
          }.buttonStyle(.plain)
        }
